...
-- 1.5 Crear una funcion que tome como parametro el id de un cliente y retorne
--	   el nombre del pais del que procede dicho cliente
CREATE OR ALTER FUNCTION obtener_pais_cliente (@id_cliente INT)
RETURNS VARCHAR(32)
AS BEGIN
	DECLARE @id_pais INT;
	DECLARE @pais VARCHAR(32);
	SELECT @id_pais = id_pais FROM CLIENTE WHERE id = @id_cliente;
	SELECT @pais = pais FROM PAIS WHERE id = @id_pais;
	RETURN @pais;
END;

SELECT dbo.obtener_pais(7);
SELECT id, nombre, documento, id_pais, dbo.obtener_pais(id) 'pais' 
FROM CLIENTE;
...
AS BEGIN
	DECLARE @total MONEY;
	SELECT @total = ISNULL(SUM(S.precio) * DATEDIFF(DAY,R.checkin,R.checkout),0)
	FROM RESERVA R 
		LEFT JOIN EXTRA X 
			ON R.id = X.id_reserva
		LEFT JOIN SERVICIO S
			ON S.id = X.id_servicio
	WHERE R.id = @id_reserva
	GROUP BY R.id, R.checkin, R.checkout
	ORDER BY R.id ASC;
	RETURN @total;
END;

SELECT *, dbo.subtotal_servicio(id) 'subtotal servicio' FROM RESERVA;
...
AS BEGIN
	DECLARE @total MONEY;
	SELECT @total = H.precio * DATEDIFF(DAY, R.checkin, R.checkout)
	FROM RESERVA R 
		INNER JOIN HABITACION H 
			ON H.id = R.id_habitacion
	WHERE R.id = @id_reserva;
...
SELECT *, dbo.subtotal_habitacion(id) 'subtotal habitacion' FROM RESERVA;


-- TOTAL
CREATE OR ALTER FUNCTION RESERVA_DETALLE ()
RETURNS TABLE
AS RETURN(
	SELECT *, 
	dbo.subtotal_servicio(id) 'subtotal servicio', 
	dbo.subtotal_habitacion(id) 'subtotal habitacion',
	dbo.subtotal_servicio(id) + dbo.subtotal_servicio(id) 'total'
	FROM RESERVA
);
